...
twitch_update() { curl -H "Accept: application/vnd.twitchtv.v3+json" -H "Authorization: OAuth $(<~/genesis/.twitch_api_token)" -d "channel[status]=$2&channel[game]=$1" -X PUT https://api.twitch.tv/kraken/channels/narthorn }
twitch() {
	twitch_update $1 $2
	ffmpeg -f x11grab -framerate 30 -s 1920x1200 -i ":0.0+1680,0" -f pulse -i default -s 1200x800 -c:v libx264 -g 30 -keyint_min 15 -b:v 3000k -minrate 3000k -maxrate 3000k -bufsize 3000k -pix_fmt yuv420p -preset ultrafast -tune zerolatency -c:a aac -strict -2 -f flv "rtmp://live.twitch.tv/app/$(<~/genesis/.twitch_key)"
}
