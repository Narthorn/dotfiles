...
twitch_update() { curl -H "Accept: application/vnd.twitchtv.v3+json" -H "Authorization: OAuth $(<~/genesis/keys/.twitch_api_token)" -d "channel[status]=$2&channel[game]=$1" -X PUT https://api.twitch.tv/kraken/channels/narthorn }
twitch() {
	twitch_update $1 $2
