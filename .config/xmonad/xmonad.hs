...
conf = defaultConfig {
    ...
    keys               = \(XConfig {modMask = modMask}) -> M.fromList $
                            ...
                            , ((modMask .|. controlMask,  xK_Print), spawn "bash -c 'maim -su | ~/dev/scripts/pyclip CLIPBOARD text/html <(echo \"<img src=\\\"$(date +%s).png\\\"/>\") image/png'") -- Copy timestamped screenshot to clipboard in select mode.
