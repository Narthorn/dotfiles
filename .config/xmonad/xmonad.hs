...
conf = defaultConfig {
    ...
    keys               = \(XConfig {modMask = modMask}) -> M.fromList $
                            ...
                            , ((modMask .|. controlMask,  xK_Print), spawn "xclip -selection clipboard </dev/null; maim -su | xclip -selection clipboard -t image/png") -- Copy screenshot to clipboard in select mode.
